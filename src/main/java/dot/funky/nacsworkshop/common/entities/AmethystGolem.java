...
public class AmethystGolem extends AbstractGolem {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new TemptGoal(this, 1.0F, Ingredient.of(ItemTags.create(new ResourceLocation(NacsWorkshop.MODID, "amethyst_fragment"))), false));
        ...
        if (itemstack.is(AMETHYST_FRAGMENT) && (this.getHealth() < this.getMaxHealth() || getBud() < 4)) {
            ...
            if (this.level.isClientSide) {
                for (int i = 0; i < 7; ++i) {
                    ...
                    this.level.addParticle(new ItemParticleOption(ParticleTypes.ITEM, new ItemStack(Items.AMETHYST_SHARD) ), this.getRandomX(1.0D), this.getRandomY() + 0.5D, this.getRandomZ(1.0D), d0, d1, d2);
